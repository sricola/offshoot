...
| Settling-flush checksum-compare suppression (skip the mandatory first-open full snapshot when content is provably unchanged since the last one) | **deliberately deferred** | Every session's first auto-flush after `open` still uploads a full snapshot unconditionally — even a session that writes nothing — sized to the database (measured 541.9MB for a 512MB db; see docs/benchmarks.md's "Settling-flush cost"). Closing the startup-rebase race this forces requires the first flush to be a real re-baseline; a checksum-compare-and-skip refinement was scoped at Task 2 but not built this pass — see [Milestone 2 follow-up](../ROADMAP.md#milestone-2--safe-by-default-for-agents) |
| Sidecar refresh on clean Close (rewrite the checkout's `.sum` sidecar to the branch's current head txid when a session closes cleanly, not just on `Checkout`/`Checkpoint`/`Rollback`/`Promote`) | **deliberately deferred** | The settling flush above advances the ref's head txid in the store ~30s after `open` under the default `-flush-every`, but `Session.Close` never calls `writeSum` (`internal/ops/ops.go`), so any session that outlives its own settling flush leaves the checkout's sidecar stale the instant it closes. The next `session open`'s `Checkout` then sees `checkoutState` report `"stale"` and re-materializes — a full disk copy plus a stranded descriptor (see `internal/dbfile`'s package comment) — even though the on-disk bytes may already match head. Not scoped or built this pass; refreshing the sidecar on a clean Close would make reopen-after-settling stay flat the same way reopen-before-settling already does — see [Milestone 2 follow-up](../ROADMAP.md#milestone-2--safe-by-default-for-agents) |
