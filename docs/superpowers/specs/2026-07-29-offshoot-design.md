...
- **Demo:** an agent (via the MCP server) forks a 10GB database three ways, attempts three migration strategies in parallel, two destroy their copies, the test suite picks the winner, `offshoot promote` — headlined honestly: *"forking a 10GB database took ~40ms"* (local reflink fork; bucket-durable in the background — and the demo says so). *(Superseded note: the "bucket-durable in the background" caveat described the removed async-upload model. Under copy-on-write ([2026-08-07-offshoot-copy-on-write-design.md](2026-08-07-offshoot-copy-on-write-design.md)) a shared fork is durable the moment its base pointer and ref land — there is no async upload or pending window to caveat; see §Fork mechanics and cost above.)*
