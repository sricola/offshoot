...
private reporting to the maintainer: open a private security advisory
(repo **Security** tab → **Report a vulnerability**) even though this isn't a
security bug — it's the private, maintainer-only channel this repo has, and
using it for a conduct report works the same way. If that's not available to
you, email the maintainer directly (see SECURITY.md for contact details).
